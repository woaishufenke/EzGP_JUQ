###Example 4.3 code for the LEzGP model
...
#Choose n_s = 7 in Step 1 of LEzGP
#According to the Step 2 of LEzGP, only 163 runs (for each replication) are selected from the full data having 19683 runs 
#this training data includes 50 replications.
tradata = as.matrix(read.table("eg4_3train.txt", h =T))
#dim(tradata)
#head(tradata)

testdata = as.matrix(read.table("eg4_3test.txt", h =T))
#head(testdata)
#dim(testdata)

### EEzGP function in LEzGP method with parallel computing for relications. 
parafunc <- function(ii, p, q, m, n, n2, tau)
{
  ...
  eval_f_list <- function(parv) {
    ...
    grad_cor <- function(h,l)
    {
      ...
      der_m = matrix(0,n,n)
      Rtemp_cor <- apply(cbind(dat[rcoord[, 1], ], dat[rcoord[, 2], ]), 1, FUN = gradf_corhs, h=h, l=l)
      der_m[rcoord] <- Rtemp_cor
      der_m <- der_m + t(der_m)
      diag(der_m) = 0
      result = sum(diag(invc %*% der_m)) - t(y-mu) %*% invc %*% der_m %*% invc %*% (y-mu)
    ...
    return( list( "objective" = MLE_result,
                  "gradient" = c(grad_var0(), grad_var(1), grad_var(2), grad_var(3),
                                 grad_var(4), grad_var(5), grad_var(6),
                                 grad_var(7), grad_var(8), grad_var(9),
                                 grad_cor0(1), grad_cor0(2), grad_cor0(3),
                                 grad_cor0(4), grad_cor0(5), grad_cor0(6),
                                 grad_cor0(7), grad_cor0(8), grad_cor0(9),
                                 grad_cor(1,2), grad_cor(1,3),
                                 grad_cor(2,2), grad_cor(2,3),
                                 grad_cor(3,2), grad_cor(3,3),
                                 grad_cor(4,2), grad_cor(4,3),
                                 grad_cor(5,2), grad_cor(5,3),
                                 grad_cor(6,2), grad_cor(6,3),
                                 grad_cor(7,2), grad_cor(7,3),
                                 grad_cor(8,2), grad_cor(8,3),
                                 grad_cor(9,2), grad_cor(9,3)) ))
  }
...
###output results
###output results
write.table(as.vector(rmser), file = "eg4_3_lezgp_rmse.txt", sep="\t")
write.table(as.vector(nser), file = "eg4_3_lezgp_nse.txt", sep="\t")
